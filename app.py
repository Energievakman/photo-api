...
<body>
  ...
  <form action="/process-photos" method="post" enctype="multipart/form-data">
    ...
    <p class="hint">
      ZIP-upload: de mapstructuur blijft behouden. Alleen .jpg, .jpeg, .png en .webp worden omgezet naar gecomprimeerde .jpg.
      PDF's, Word-bestanden, Excel-bestanden en andere bestanden blijven zoals ze zijn.
    </p>
  ...
  <hr>
  <p>Voor Softr gebruik je:</p>
  <code>/zip-from-softr/&lt;record_id&gt;</code>
</body>
</html>
"""
...
def process_zip_bytes(zip_bytes, output_zip, used_names, max_width=1200, quality=65):
    try:
        with ZipFile(zip_bytes, "r") as input_zip:
            for info in input_zip.infolist():
                original_path = safe_filename(info.filename)
                ...
                if is_image(original_path):
                    try:
                        ...
                        original_path = unique_zip_name(original_path, used_names)
                        output_zip.writestr(original_path, data)
                        output_zip.writestr(f"fout_{os.path.basename(original_path)}.txt", f"Kon afbeelding niet comprimeren: {str(e)}")
                else:
            ...
            try:
                ...
                if source.get("type") == "upload":
                    ...
                    if is_zip(original_name):
                        file.stream.seek(0)
                        process_zip_bytes(BytesIO(file.stream.read()), zip_file, used_names, max_width=max_width, quality=quality)
                    ...
                    if is_zip(url_name):
                        process_zip_bytes(file_bytes, zip_file, used_names, max_width=max_width, quality=quality)
                ...
                zip_file.writestr(f"fout_{index:03d}.txt", f"Kon bestand {index} niet verwerken: {str(e)}")
