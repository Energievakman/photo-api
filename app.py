...
<body>
  ...
  <form action="/upload-for-record/{{record_id}}" method="post" enctype="multipart/form-data">
    ...
    <p id="loadingMessage" style="display:none;color:#0a66c2;font-weight:bold;margin-top:15px;">
      ⏳ Even geduld a.u.b. Het dossier wordt parallel verwerkt...
    </p>
  </form>
<script>
document.querySelector("form").addEventListener("submit", function () {
...
def process_zip_bytes(zip_bytes, output_zip, used_names, written_paths, max_width=1600, quality=75):
    ...
    Afbeeldingen worden parallel gecomprimeerd voor meer snelheid.
    Niet-afbeeldingen worden direct gekopieerd.
    """
    try:
        with ZipFile(zip_bytes, "r") as input_zip:
            ...
            # Eerst ZIP uitlezen. ZipFile tegelijk lezen vanuit meerdere threads is niet ideaal.
            for info in input_zip.infolist():
                ...
                if is_image(original_path):
                    image_jobs.append((original_path, data))
                else:
                    direct_entries.append((original_path, data))

            # Gewone bestanden kopiëren.
            for original_path, data in direct_entries:
                output_path = unique_zip_name(original_path, used_names)
                output_zip.writestr(output_path, data)
                written_paths.append(output_path)

            # Afbeeldingen parallel comprimeren.
            if image_jobs:
                workers = max(1, min(IMAGE_WORKERS, len(image_jobs)))
                ...
                with ThreadPoolExecutor(max_workers=workers) as executor:
                    ...
                    # ZIP schrijven blijft bewust sequentieel; ZipFile schrijven is niet thread-safe.
                    for result in results:
