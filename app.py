  ...
  <form action="/process-photos" method="post" enctype="multipart/form-data">
    ...
    <label>
      ...
      <select name="max_width">
        <option value="1200">1200 px - snel / klein</option>
        <option value="1600" selected>1600 px - aanbevolen</option>
    ...
    <label>
      Kwaliteit:
      <select name="quality">
        <option value="65">65 - klein bestand</option>
        <option value="78" selected>78 - aanbevolen</option>
...
def compress_image_from_filelike(file_obj, max_width=1600, quality=78):
...
def compress_upload_image(file_storage, max_width=1600, quality=78):
...
def process_zip_bytes(zip_bytes, output_zip, used_names, written_paths, max_width=1600, quality=78):
    try:
...
def build_zip_from_sources(sources, max_width=1600, quality=78):
    zip_buffer = BytesIO()
    used_names = set()
    written_paths = []
...
def process_photos():
    ...
    try:
        ...
        max_width = 1200
    try:
        quality = int(request.form.get("quality", 78))
    except ValueError:
        quality = 65
...
def zip_from_softr(record_id):
    try:
        ...
        max_width = 1200
    try:
        quality = int(request.args.get("quality", 78))
    except ValueError:
        quality = 65
