...
<body>
  ...
  <p>Upload hier het grote projectdossier direct naar Render. Dit omzeilt de 128 MB limiet van Softr.</p>
  ...
  <form action="/upload-for-record/{{record_id}}" method="post" enctype="multipart/form-data">
    ...
    <p class="hint">
      Na verwerking wordt het kleine ZIP-bestand klaargezet als download. Als <b>SOFTR_OUTPUT_FILE_FIELD</b>
      is ingesteld, probeert de app ook automatisch het Softr-record te vullen met de downloadlink.
    </p>
...
def update_softr_record_with_zip_url(record_id, file_url):
    """
    Probeert het Softr file field te vullen met de download-URL.
    Hiervoor moet in Render staan:
    SOFTR_OUTPUT_FILE_FIELD = field ID van het outputveld, bijvoorbeeld Gecomprimeerd.
    """
    if not SOFTR_API_KEY or not SOFTR_DATABASE_ID or not SOFTR_TABLE_ID:
    ...
    payload_options = [
        {"fields": {SOFTR_OUTPUT_FILE_FIELD: [{"url": file_url, "name": OUTPUT_ZIP_NAME}]}},
        {"fields": {SOFTR_OUTPUT_FILE_FIELD: file_url}},
        {SOFTR_OUTPUT_FILE_FIELD: [{"url": file_url, "name": OUTPUT_ZIP_NAME}]},
        {SOFTR_OUTPUT_FILE_FIELD: file_url},
    ]
    ...
    return False, f"ZIP is gemaakt, maar automatisch koppelen aan Softr lukte niet: {last_error}"
...
def upload_for_record(record_id):
    """
    Uploadpagina voor Softr:
    Softr knop opent /upload-for-record/<record_id>.
    De gebruiker uploadt de grote ZIP rechtstreeks naar Render.
    Render comprimeert en probeert het resultaatveld in Softr te vullen.
    """
